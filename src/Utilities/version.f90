...
! ------------------------------------------------------------------------------
    ...
    character(len=*), intent(in), optional :: nrprocstr !PAR
    ! -- local
...
! ------------------------------------------------------------------------------
    ...
    call write_centered('VERSION '//VERSION, iheader_width, iunit=iout,         &
      force_write=.true.) !PAR
    if (present(nrprocstr)) then !PAR
      call write_centered('***RUNNING IN PARALLEL MODE WITH '                   & !PAR
      //trim(nrprocstr)//' MPI PROCESSES***', iheader_width,                    & !PAR
      iunit=iout, force_write=.true.) !PAR
    end if !PAR
    !
    ...
    call write_centered(' ', iheader_width, iunit=iout, force_write=.true.) !PAR
