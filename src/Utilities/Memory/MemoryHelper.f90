...
contains
  ...
  function create_mem_path(component, subcomponent) result(memory_path)
    ...
    if (present(subcomponent)) then
      if (len_trim(subcomponent) > 0) then !PAR
        memory_path = trim(component) // memPathSeparator // trim(subcomponent)
      else !PAR
        memory_path = trim(component) !PAR
      end if !PAR
    else
  ...
  !! value for @par subcomponent is set to an empty string.
  !<
  subroutine split_mem_path_ff(mem_path, component, subcomponent) !PAR
    character(len=*), intent(in) :: mem_path                      !< path to the memory object
    character(len=*), intent(out) :: component     !< name of the component (solution, model, exchange)
    character(len=*), intent(out) :: subcomponent  !< name of the subcomponent (package)
  ...
  end subroutine split_mem_path_ff
